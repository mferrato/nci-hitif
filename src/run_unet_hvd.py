...
class TestCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        x, y = self.test_data
        loss, acc = self.model.evaluate(x, y, verbose=0, batch_size = b_size)
        print('\nTesting loss: {}, acc: {}\n'.format(loss, acc))
...
#    imgs_mask_train = np.expand_dims(imgs_mask_train, axis= 3)
    ...
    if augmentation_factor != None:
        ...
        i = 0     
        for batch in image_datagen.flow(imgs_train, batch_size=b_size, seed=1337): 
        ...
        i = 0     
        for batch in image_datagen.flow(imgs_mask_train, batch_size=b_size, seed=1337): 
...
def train(model, imgs_train, imgs_mask_train, b_size, initialize=None):
    ...
    callbacks = [
	#Horovod Broadcast to guarantee consistent initialization of all workers
	hvd.callbacks.BroadcastGlobalVariablesCallback(0),

	#Average metrics among workers at the end of every epoch
	hvd.callbacks.MetricAverageCallback(),

	#Scale learning rate on the first 5 epochs to improve accuracy
	hvd.callbacks.LearningRateWarmupCallback(),

	#Reduce training rate if learning rate plateaues
	ReduceLROnPlateau(monitor='val_loss', factor=0.1,patience=100, verbose=1),

	#Stops model if overfitting
	EarlyStopping(monitor='val_loss', min_delta=0.000001, patience=400, verbose=1, mode='auto'),

	#Creates training log cvs file
	CSVLogger('training.csv'),
    ]

    #Saves checkpoints only on worker 0 to prevent other workers from corrupting them
    if hvd.rank() == 0:
	callbacks.append(ModelCheckpoint(modelwtsfname, monitor='val_loss', save_best_only=True))
    ...
    # Fraction of the training data you want to use as the validation data
    validation_fraction = 0.10

    # Determine how many batches are there in train and test set
    train_steps = (len(imgs_train) * (1 - validation_fraction)) // b_size
    val_steps = (len(imgs_train) * validation_fraction) // b_size
    ...
    # 1 / N batches of validation data on every worker, where N is the number of workers.
    # Over-sampling of validation data helps to increase probability that every validation
    # example will be evaluated.
 
    generator = ImageDataGenerator(validation_split = validation_fraction)

    train_gen = generator.flow(imgs_train, imgs_mask_train, batch_size=b_size, subset="training")
    val_gen = generator.flow(imgs_train, imgs_mask_train, batch_size=b_size, subset="validation")

    #train_steps = 820 // b_size
    #val_steps = 92 // b_size
    ...
    #                 callbacks=callbacks,
    #                 steps_per_epoch=int(820 // hvd.size()),
    #                 validation_steps=int(92 // hvd.size()))
...
def predict(model, weights, images, args):
    ...
    imgs_mask_test = model.predict(images, batch_size = b_size,verbose=1)
...
def evaluate_params(args): 
    ...
    history_callback = train(model, images, masks, args.batch_size, initialize=args.initialize)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', default=1, type = int, dest='batch_size', help="")
